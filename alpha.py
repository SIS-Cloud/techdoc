...
# สร้าง class ชื่อ MyForm สำหรับสร้าง Form บนหน้าเว็บ
    ...
    # สร้างตัวแปร Tenantid สำหรับสร้างช่องกรอกข้อมูลที่เป็น text บน Form
    ...
    select_template = SelectField("เลือก Template Techdoc",
                                  choices=[('1','1101-XXXX S3 Storage'),
                                           ...
                                           ('5','1103-XXXX Vmware IaaS + CrowdStrike'),
                                           ('6','1103-XXXXDR Vmware DRaaS (VCC01)'),
                                           ('7','1103-XXXXDR Vmware DRaaS (VCC04)'),
                                           ('8','1105-XXXX Nutanix IaaS'),
                                           ('9','1105-XXXX Nutanix IaaS + CrowdStrike'),
                                           ('10','1107-XXXX Magicbox'),
                                           ('11','1401-XXXX Wasabi Storage'),
                                           ('12','1115-XXXX Virtuozzo')])
                                        #    ('13','1002-XXXX Veeam Agent IDC'),
                                        #    ('14','1002-XXXX Veeam Backup Standard IDC'),
                                        #    ('15','1003-XXXX Vmware IaaS IDC'),
                                        #    ('16','1003-XXXXDR Vmware DRaaS IDC'),
                                        #    ('17','1005-XXXX Nutanix IaaS IDC')])
...
def index():
    ...
    # สร้างตัวแปร form เท่ากับ class MyForm
        ...
        # สร้างเงื่อนไข ถ้ามีการรับข้อมูลมาจากฟอร์ม ให้ทำอย่างไร
            ...
            # สร้างเงื่อนไข ถ้ามีการกด Submit บน Form ให้ทำอย่างไร
                ...
                # โดยใช้ session ในการกระจายค่าตัวแปรต่างๆ ไปยังแต่ละหน้าเว็บที่ต้องการเรียกใช้งาน

                form.tenantid.data = ""
                form.select_template.data = ""
                # ทำให้ข้อความจาก Form กลายเป็นค่าว่างหลังกด submit

                con_string = "driver=SQL SERVER;server=27.254.204.72;database=Cloud;UID=cloudapi;PWD=Cloudapi1234!"
                # Connect Database Server โดยการกำหนด Parameter ให้ถูกต้องในการเชื่อมต่อ
                ...
                # ใช้ SQL Statement ในการ Query ข้อมูลบางส่วน สำหรับการทำตาราง Item Description ใน Techdoc
                ...
                with pyodbc.connect(con_string, autocommit= True) as con:
                    ...
                    con.close()
                    # Execute SQL Statement และนำข้อมูลมา save เป็น File Excel
                ...
                test = df.loc[[x]]
                print(test)
                # อ่านค่า Dataframe ที่ตำแหน่ง Index = X (เลข Tenant)
                ...
                if status == 'POC':
                    ...
                    pocstartdate = pd.to_datetime(df.loc[[x],'poc_startdate'].values[0]).strftime("%d.%m.%y")
                    pocenddate = pd.to_datetime(df.loc[[x],'poc_enddate'].values[0]).strftime("%d.%m.%y")
                # เปลี่ยนข้อมูล String เป็น Format Datetime ตามเงื่อนไขของ Status
                ...
                print(context)
                # สร้างตัวแปร context ทำ Dictionary กับตัวแปรที่เก็บข้อมูลจาก Dataframe ไปใช้บน textdoc template

                def format_template():
                # สร้าง function format_template() เพื่อเรียกใช้โดยง่ายและย่อบรรทัด coding
                    if status == 'POC':
                    ...
                    # สร้างเงื่อนไข update ข้อมูลและ render ข้อมูลลงไฟล์ techdoc ตาม Status
                ...
                if session['select_template'] == '1':
                    ...
                    print("1105 Template : " + session['select_template'])
                    y = "1105-xxxx + CrowdStrike"
                    format_template()
                elif session['select_template'] == '10':
                    print("1107 Template : " + session['select_template'])
                    y = "1107-xxxx"
                    format_template()
                elif session['select_template'] == '11':
                    print("1401 Template : " + session['select_template'])
                    y = "1401-xxxx"
                    format_template()
                elif session['select_template'] == '12':
                    print("1115 Template : " + session['select_template'])
                    y = "1115-xxxx"
                    format_template()
                # elif session['select_template'] == '13':
                #     print("1002 Template : " + session['select_template'])
                #     y = "1002-XXXX (Veeam Agent)"
                #     format_template()
                # elif session['select_template'] == '14':
                #     print("1002 Template : " + session['select_template'])
                #     y = "1002-XXXX (Veeam Backup Standard)"
                #     format_template()
                # elif session['select_template'] == '15':
                #     print("1003 Template : " + session['select_template'])
                #     y = "1003-XXXX"
                #     format_template()
                # elif session['select_template'] == '16':
                #     print("1003DR Template : " + session['select_template'])
                #     y = "1003-XXXXDR"
                #     format_template()
                # elif session['select_template'] == '17':
                #     print("1005 Template : " + session['select_template'])
                #     y = "1005-XXXX"
                #     format_template()
                # สร้างเงื่อนไขตาม select_template เพื่อนำค่าตัวแปร y ไปใช้ใน function format_template()

            return render_template('success.html',form=form)
            # ส่งค่า template จากไฟล์ success.html และตัวแปร form ไปแสดงผลบน Form หน้าเว็บ

    except KeyError:
    # กำหนดให้ยกเว้น Code Error เมื่อเจอ KeyError และแสดงข้อความ "No Tenant ID แทน
        print("No Tenant ID : " + x)
        error = "No Tenant ID : " + x
        return render_template('index.html',form=form, error=error)
        # ส่งค่า template จากไฟล์ index.html และตัวแปร form, eror ไปแสดงผลบน Form หน้าเว็บ

    return render_template('index.html',form=form)
    # ส่งค่า template จากไฟล์ index.html และตัวแปร form ไปแสดงผลบน Form หน้าเว็บ

if __name__ == "__main__":
    app.run(debug=True,use_reloader=True)
    # สั่ง app run โดยเปิด debug mode บนหน้าเว็บและทำการ auto reload ทุกครั้งที่กด save
    # Production Server ให้กำหนด host= และ port=